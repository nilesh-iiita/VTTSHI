...
```{r}
setwd("/data/user/nileshkr/2022/Prj29_Star_protocol/STAR/RNASeq/Herpes_GSE124118")
```
...
```{r}
mainDir = getwd()
subDir_img = "00_DEDeq2_Images_HSV"
subDir_data = "00_DEDeq2_Data_HSV"
...
```{r Read_data}
Read_count <- "GSE124118_human_counts.txt"
Meta <- 'DESeq_meta.csv'
cts <- as.matrix(read.csv(Read_count,sep="\t",row.names="Ensemble"))
coldata <- read.csv(Meta, row.names=1)
#coldata <- coldata[,c("hsv_cells")]
coldata$hsv_cells <- factor(coldata$hsv_cells)
...
```{r}
dds$hsv_cells <- relevel(dds$hsv_cells, ref = "lung")
```
...
#res <- results(dds, contrast=c("hsv_cells", "Skin", "lung"))
resultsNames(dds)
```
...
## Apeglm
```{r}
resLFC <- lfcShrink(dds, coef="hsv_cells_Skin_vs_lung", type="apeglm")
...
ENSEMBL_IDS = unlist(lapply(ENSEMBL_IDS,  sub, pattern = "\\.\\d+$", replacement = ""))
...
```{r}
res$Symbol <- convertIDs(ENSEMBL_IDS, "ENSEMBL","SYMBOL", org.Hs.eg.db)
res$Alias <- convertIDs(ENSEMBL_IDS, "ENSEMBL", "ALIAS", org.Hs.eg.db)
res$ENTREZID <- convertIDs(ENSEMBL_IDS, "ENSEMBL", "ENTREZID", org.Hs.eg.db)
res$GO  <- convertIDs(ENSEMBL_IDS, "ENSEMBL", "GO", org.Hs.eg.db)
res$Path  <- convertIDs(ENSEMBL_IDS, "ENSEMBL", "PATH", org.Hs.eg.db)
res$Evidance  <- convertIDs(ENSEMBL_IDS, "ENSEMBL", "EVIDENCE", org.Hs.eg.db)
res$PMID  <- convertIDs(ENSEMBL_IDS, "ENSEMBL", "PMID", org.Hs.eg.db)
res$UNIPROT  <- convertIDs(ENSEMBL_IDS, "ENSEMBL", "UNIPROT", org.Hs.eg.db)
#res$ENSEMBL  <- convertIDs(ENSEMBL_IDS, "ENSEMBL", "ENSEMBL", org.Hs.eg.db)
...
Plotcount = plotCounts(dds, gene=which.min(res$padj), intgroup="hsv_cells")
Plotcount
```
...
```{r}
data <- plotCounts(dds, gene=topGene, intgroup="hsv_cells", returnData=TRUE)
ggplot2::ggplot(data, ggplot2::aes(x=hsv_cells, y=count, color=hsv_cells)) +
...
```{r}
ggplot2::ggplot(data, ggplot2::aes(x=hsv_cells, y=count, fill=hsv_cells)) +
  ggplot2::scale_y_log10() + 
  ggplot2::geom_dotplot(binaxis="y", stackdir="center") 
```

```{r}
data <- plotCounts(dds, gene=topGene, intgroup=c("hsv_cells"), returnData=TRUE)
COUNT_PLOT = ggplot2::ggplot(data, ggplot2::aes(x=hsv_cells, y=count, fill=hsv_cells)) +
  ggplot2::scale_y_log10() + 
  ggplot2::geom_dotplot(binaxis="y", stackdir="center")
COUNT_PLOT
```
...
write.csv(as.data.frame(resOrdered), 
          file=paste(subDir_data,"hsv_cells_Skin_vs_lung.csv", sep = "/"))
```
...
resMFType <- results(ddsMF,
                     contrast=c("hsv_cells", "Skin", "lung"))
head(resMFType)
```
...
df <- as.data.frame(colData(dds)[,c("hsv_cells")])
...
select <- order(rowMeans(counts(dds,normalized=TRUE)),
                decreasing=TRUE)[1:20]
df <- as.data.frame(colData(dds)["hsv_cells"])
HeatMap = pheatmap(assay(ntd)[select,], cluster_rows=FALSE, show_rownames=FALSE,
...
```{r}
PCA_1 = plotPCA(vsd, intgroup=c("hsv_cells"))
...
```{r}
library("ggplot2")
pcaData <- plotPCA(vsd, intgroup=c("hsv_cells"), returnData=TRUE)
percentVar <- round(100 * attr(pcaData, "percentVar"))
PCA_2 = ggplot(pcaData, aes(PC1, PC2, color=hsv_cells, shape=hsv_cells)) +
  geom_point(size=3) +
